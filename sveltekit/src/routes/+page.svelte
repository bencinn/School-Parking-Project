<script>
    ...
    let full1 = 0, full2 = 0, full3 = 0, full4 = 0, full5 = 0, full6 = 0, full7 = 0;

    for(let i = 0; i < Parking_lot.length; i++) {
        if (1 <= parseInt(Parking_lot[i].parked_where) && parseInt(Parking_lot[i].parked_where) <= 13) {
            full1 ++;
        } else if (14 <= parseInt(Parking_lot[i].parked_where) && parseInt(Parking_lot[i].parked_where) <= 37) {
            full2 ++;
        } else if (38 <= parseInt(Parking_lot[i].parked_where) && parseInt(Parking_lot[i].parked_where) <= 64) {
            full3 ++;
        } else if (65 <= parseInt(Parking_lot[i].parked_where) && parseInt(Parking_lot[i].parked_where) <= 73) {
            full4 ++;
        } else if (74 <= parseInt(Parking_lot[i].parked_where) && parseInt(Parking_lot[i].parked_where) <= 94) {
            full5 ++;
        } else if (95 <= parseInt(Parking_lot[i].parked_where) && parseInt(Parking_lot[i].parked_where) <= 108) {
            full6 ++;
        } else if (109 <= parseInt(Parking_lot[i].parked_where) && parseInt(Parking_lot[i].parked_where) <= 110) {
            full7 ++;
        }
    };

    let v1 = 13, v2 = 24, v3 = 27, v4 = 9, v5 = 21, v6 = 14, v7 = 2;
